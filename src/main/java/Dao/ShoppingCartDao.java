...
/**
 * @author 林丽媛
 * 创建时间： 2017年6月18日 上午10:12:02
 */
public interface ShoppingCartDao extends BaseDao<ShoppingCart>{
	@Insert("insert into shoppingcart(user_id,good_id,number,size) values (#{user.id},#{good.id},#{number},#{size})")
	...
	@Update("update shoppingcart set number=#{number},size=#{size} where id=#{id}")
